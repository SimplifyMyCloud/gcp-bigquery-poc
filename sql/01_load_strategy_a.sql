-- Strategy A: Hourly Partition — generate ~52.56M rows directly in BQ
-- Split into 3 chunks because BQ limits DML to 4,000 partitions per statement,
-- and 365 days × 24 hours = 8,760 hourly partitions.

TRUNCATE TABLE `simplifymycloud-dev.cell_network_poc.metrics_hourly_part`;

-- ─── Chunk 1: Jan – Apr (120 days × 24h = 2,880 partitions) ────────────────
  ...
  cells AS (
    SELECT
      ...
      CASE MOD(cell_num - 1, 3)
        WHEN 0 THEN 'us-west' WHEN 1 THEN 'us-central' ELSE 'us-east'
      END AS region_id,
...
-- ─── Chunk 2: May – Aug (123 days × 24h = 2,952 partitions) ────────────────
...
WITH
  timestamps AS (
    SELECT ts FROM UNNEST(GENERATE_TIMESTAMP_ARRAY(
      TIMESTAMP '2025-05-01 00:00:00 UTC',
      TIMESTAMP '2025-08-31 23:59:00 UTC',
      INTERVAL 1 MINUTE
    )) AS ts
  ),
  cells AS (
...
-- ─── Chunk 3: Sep – Dec (122 days × 24h = 2,928 partitions) ────────────────
